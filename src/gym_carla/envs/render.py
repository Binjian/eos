...
class MapImage(object):
    def __init__(self, carla_world, carla_map, pixels_per_meter):
        ...
        self.big_map_surface = pygame.Surface(
            (width_in_pixels / 30, width_in_pixels / 30)
        ).convert()
...
class BirdeyeRender(object):
    ...
    def _render_hist_actors(
        self, surface, actor_polygons, actor_type, world_to_pixel, num
    ):
    ...
    def render(self, display, render_types=None):
        ...
        self.render_actors(
            self.actors_surface, self.vehicle_polygons, self.walker_polygons
        )

        self.waypoints_surface.fill(COLOR_BLACK)
        self.render_waypoints(
            self.waypoints_surface, self.waypoints, self.map_image.world_to_pixel
        )
        ...
        if self.hero_actor is not None:
            ...
            translation_offset = (
                ...
                + hero_front.x * self.params["pixels_ahead_vehicle"],
                (
                    hero_location_screen[1]
                    - self.hero_surface.get_height() / 2
                    + hero_front.y * self.params["pixels_ahead_vehicle"]
                ),
            )
            ...
            center_offset = (
                abs(display.get_height() - self.surface_size) / 2 * scale_factor,
                0,
            )
