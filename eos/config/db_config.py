...
class Record(TypedDict):
    ...
    _id: NotRequired[
        ObjectId
    ]  # for record, _id is generated by pymongo, not required
    timestamp: datetime
    plot: Plot
    observation: Dict[str, list[Any]]
...
class Episode(TypedDict):
    ...
    timestamp: datetime
    plot: Plot
    history: list[Dict[str, list[Any]]]  # length of the list is the length of the episode
...
record_schemas = dict(
    zip([schema.NAME for schema in rec_schema_list], rec_schema_list)
)
...
def get_db_config(db_key: str) -> DB_CONFIG:
    ...
    if (
        ...
        assert (
            len(account_server) == 1 and len(flat_account_server) == 2
        ) or (
        ...
        if len(account_server) == 1:
            ...
            assert (
                ...
                and db_config.Port == flat_account_server[1]
